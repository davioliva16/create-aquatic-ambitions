...
public class CAAIcons extends AllIcons {
    ...
    public static final ResourceLocation ICON_ATLAS = CreateAquaticAmbitions.asResource("textures/gui/icons.png");
    private static int x = 0, y = -1;
    private int iconX;
    private int iconY;

    public static final CAAIcons
            I_EVERYONE = newRow(),
            I_PLAYERS = next(),
            I_FRIENDLY_MOBS = next(),
            I_PLAYERS_FRIENDLY_MOBS = next(),
            I_MONSTERS = next(),

            I_CONDUIT_POWER = newRow(), //Used on ponder.
            I_FIRE_RES = next();

    public CAAIcons(int x, int y) {
        super(x, y);
        iconX = x * 16;
        iconY = y * 16;
    }

    private static CAAIcons next() {
        return new CAAIcons(++x, y);
    }

    private static CAAIcons newRow() {
        x = 0;
        y++;
        return new CAAIcons(x, y);
    }
